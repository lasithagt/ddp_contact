function [x,u]= demo_robot_7dof_contact
...
q0       = [0.416877955598325, 0.484239417008371, 0.135996055030439, 1.471976911034206, 0.074352122183208, 0.991357480393416, ...
            -0.578151574356318];
x0       = [q0 zeros(1,10)]';                     % initial state
...
p            = [0.1, 0.1, 0.9]';
pose         = [[eye(3);0 0 0] [p;1]];
theta0       = 0 + zeros(7,1);
[xd_, s]     = IKinSpace_modified(Slist, M_K, pose, theta0, 0.01,0.01);
xd_          = mod(xd_,2*pi);

global xd
xd = [repmat(xd_, 1, 501);zeros(9,501);ones(1,501)];
...
function c = robot_cost(x, u, i)
    ...
    cu  = 1e-3*[1 1 1 1 1 1 1];         % control cost coefficients

    cf  = [.1 .1 .1 .1 .1 .1 .1 0*ones(1,7) ones(1,3)];    % final cost coefficients
    pf  = [.01 .01 .01  1 0.1*ones(1,10) ones(1,3)]';            % smoothness scales for final cost

    cx  = 1e-1 * [1 1 1 1 1 1 1 0*ones(1,10)];              % running cost coefficients
    px  = [.1 .1 .1 .1 .1 .1 .1 0*ones(1,10)]';             % smoothness scales for running cost
