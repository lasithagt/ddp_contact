...
function [x, u, L, Vx, Vxx, cost, trace, stop] = iLQG_ADMM_CEN(DYNCST, DYNCST_primal, x0, u0, rhao, x_bar, c_bar, u_bar, Op)
...
if size(x0,2) == 1
    diverge = true;
    for alpha = Op.Alpha
        [x,un,cost]  = forward_pass(x0(:,1),alpha*u,[],[],[],1,DYNCST,DYNCST_primal,lims,[],rhao,x_bar, c_bar,u_bar);
...
for iter = 1:Op.maxIter
    ...
    if backPassDone
        ...
        if Op.parallel  % parallel line-search
            [xnew,unew,costnew] = forward_pass(x0 ,u, L, x(:,1:N), l, Op.Alpha, DYNCST,DYNCST_primal,lims,Op.diffFn,rhao,x_bar,c_bar,u_bar);
            ...
            for alpha = Op.Alpha
                [xnew,unew,costnew]   = forward_pass(x0 ,u+l*alpha, L, x(:,1:N),[],1,DYNCST,DYNCST_primal,lims,Op.diffFn,rhao,x_bar, c_bar,u_bar);
...
function [cx,cu,cxx,cxu,cuu] = preprocessing(x,u,cx,cu,cxx,cxu,cuu,rhao,x_bar,c_bar,u_bar)
    N = size(cx,2);
    n = numel(cx)/N;
    m = numel(cu)/N;
    rhao1 = rhao(1);
...
function [xnew,unew,cnew] = forward_pass(x0,u,L,x,du,Alpha,DYNCST,DYNCST_primal,lims,diff,rhao,x_bar,c_bar,u_bar)
