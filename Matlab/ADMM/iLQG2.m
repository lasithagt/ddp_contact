...
function [x, u, L, Vx, Vxx, cost, trace, stop] = iLQG2(DYNCST, DYNCST2, x0, u0, rho, x_bar, u_bar, Op)
...
if size(x0,2) == 1
    diverge = true;
    for alpha = Op.Alpha
        [x,un,cost]  = forward_pass(x0(:,1),alpha*u,[],[],[],1,DYNCST,DYNCST2,lims,[],rho,x_bar,u_bar);
...
for iter = 1:Op.maxIter
    ...
    if flgChange
        ...
        [cx,cu,cxx,cxu,cuu] = preprocessing(x,u,cx,cu,cxx,cxu,cuu,rho,x_bar,u_bar);
    ...
    if backPassDone
        ...
        if Op.parallel  % parallel line-search
            [xnew,unew,costnew] = forward_pass(x0 ,u, L, x(:,1:N), l, Op.Alpha, DYNCST,DYNCST2,lims,Op.diffFn,rho,x_bar,u_bar);
            ...
            for alpha = Op.Alpha
                [xnew,unew,costnew]   = forward_pass(x0 ,u+l*alpha, L, x(:,1:N),[],1,DYNCST,DYNCST2,lims,Op.diffFn,rho,x_bar,u_bar);
...
function [cx,cu,cxx,cxu,cuu] = preprocessing(x,u,cx,cu,cxx,cxu,cuu,rho,x_bar,u_bar)
N = size(cx,2);
n = numel(cx)/N;
m = numel(cu)/N;
rho1 = rho(1);
rho2 = rho(2);
for i = 1:N-1
    cx(:,i) = cx(:,i) + rho1*x(:,i) - rho1*x_bar(:,i);
    cu(:,i) = cu(:,i) + rho2*u(:,i) - rho2*u_bar(:,i);
    cxx(:,:,i) = cxx(:,:,i) + rho1*eye(n);
    cuu(:,:,i) = cuu(:,:,i) + rho2*eye(m);
    cxu(:,:,i) = cxu(:,:,i);
end
cx(:,N) = cx(:,N) + rho1*x(:,N) - rho1*x_bar(:,N);
cxx(:,:,N) = cxx(:,:,N) + rho1*eye(n);


function [xnew,unew,cnew] = forward_pass(x0,u,L,x,du,Alpha,DYNCST,DYNCST2,lims,diff,roll,x_bar,u_bar)
...
for i = 1:N
    ...
    cnew(:,:,i)         = DYNCST2(xnew(:,:,i), unew(:,:,i), roll, x_bar(:,i*K1), u_bar(:,i*K1), i*K1);
end
cnew(:,:,N+1) = DYNCST2(xnew(:,:,N+1),nan(m,K,1), roll, x_bar(:,(N+1)*K1), nan(m,K,1), i);
