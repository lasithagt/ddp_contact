function [x,u]= demo_robot_Point_Mass
...
x0      = [0.5 0 0 0 0 0 0 0 0]';   % states = [position,  velocity, force]
u0      = 0.0 + zeros(3,T);         % initial controls
...
global xd
xd   = [xd_x;xd_y;zeros(6,numel(t));xd_f];
% x = zeros(4,200);
% u = ones(2,200);
...
function c = robot_cost(x, u, i)
    ...
    cu  = 1e-1*[1 1 1];         % control cost coefficients

    cf  = [1 1 0 0 0 0 1 1 1];      % final cost coefficients
    pf  = [.01 .0 .01 0 0 0 1 1 1]';    % smoothness scales for final cost

    cx  = 5e-0*[10 10 0 0 0 0 10 10 10];        % running cost coefficients
    px  = 4*[.1 .1 0 0 0 0 .1 .1 .1]';           % smoothness scales for running cost
