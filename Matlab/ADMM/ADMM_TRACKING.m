function [x, u, cost] = ADMM_TRACKING(DYNCST,DYNCST2,x0,u0,COMTask,SwingFootTask,spmode,xf,Op)
...
iter = 1;
% --- initial trajectory
[x,un,c0]  = initialtraj(x0,u0,COMTask,SwingFootTask,spmode,xf,DYNCST);
c01 = sum(c0(:));
...
%         end
    ...
    res_ulambda(:,i) = roll(2)*norm(u_bar - u_bar_old);
    res_xlambda(:,i) = roll(1)*norm(x_bar - x_bar_old);

    [~,~,cost22]  = initialtraj(x0,unew,COMTask,SwingFootTask,spmode,xf,DYNCST);
...
plot(l,res_ulambda,'DisplayName','residue u_lambda');
plot(l,res_xlambda,'DisplayName','residue x_lambda');
...
[~,~,costnew]  = initialtraj(x0,unew,COMTask,SwingFootTask,spmode,xf,DYNCST);
...
function [xnew,unew,cnew] = initialtraj(x0,u0,COMTask,SwingFootTask,spmode,xf,DYNCST)
...
cnew        = zeros(1,N+1);
for i = 1:N
    [xnew(:,i+1), cnew(:,i)]  = DYNCST(xnew(:,i), unew(:,i),COMTask(:,i),SwingFootTask(:,i),spmode,xf,i);
end
[~, cnew(:,N+1)] = DYNCST(xnew(:,N+1),nan(m,1),COMTask(:,N+1),SwingFootTask(:,N+1),spmode,xf,i);
...
function [x, u, L, Vx, Vxx, cost, trace, stop] = iLQG_TRK(DYNCST, DYNCST2, x0, u0, roll, x_bar, u_bar, COMTask,SwingFootTask,spmode,xf,Op)
%---------------------- user-adjustable parameters ------------------------
defaults = {'lims',           [],...            control limits
            'parallel',       true,...          use parallel line-search?
            'Alpha',          10.^linspace(0,-3,11),... backtracking coefficients
...
N   = size(u0, 2);          % number of state transitions
u   = u0;                   % initial control sequence
% DYNCST2  = @(x,u,roll,x_bar,u_bar,i) new_car_cost(x,u,roll,x_bar,u_bar);
...
if size(x0,2) == 1
    diverge = true;
    for alpha = Op.Alpha
        [x,un,cost]  = forward_pass(x0(:,1),alpha*u,[],[],[],1,DYNCST,DYNCST2,lims,[],roll,x_bar,u_bar,COMTask,SwingFootTask,spmode,xf);
...
for iter = 1:Op.maxIter
    ...
    if flgChange
        t_diff = tic;
        [~,~,fx,fu,fxx,fxu,fuu,cx,cu,cxx,cxu,cuu]   = DYNCST(x,[u nan(m,1)],COMTask,SwingFootTask,spmode,xf,1:N+1);
        [cx,cu,cxx,cxu,cuu] = preprocessing(x,u,cx,cu,cxx,cxu,cuu,roll,x_bar,u_bar);
    ...
    while ~backPassDone
        ...
        t_back   = tic;
        [diverge, Vx, Vxx, l, L, dV] = back_pass(cx,cu,cxx,cxu,cuu,fx,fu,fxx,fxu,fuu,lambda,Op.regType,lims,u);
    ...
    if backPassDone
        ...
        if Op.parallel  % parallel line-search
            [xnew,unew,costnew] = forward_pass(x0 ,u, L, x(:,1:N), l, Op.Alpha, DYNCST,DYNCST2,lims,Op.diffFn,roll,x_bar,u_bar,COMTask,SwingFootTask,spmode,xf);
            ...
            for alpha = Op.Alpha
                [xnew,unew,costnew]   = forward_pass(x0 ,u+l*alpha, L, x(:,1:N),[],1,DYNCST,DYNCST2,lims,Op.diffFn,roll,x_bar,u_bar,COMTask,SwingFootTask,spmode,xf);
...
for i = 1:N-1
    cx(:,i) = cx(:,i) + roll(1)*x(:,i) - roll(1)*x_bar(:,i);
    cu(:,i) = cu(:,i) + roll(2)*u(:,i) - roll(2)*u_bar(:,i);
    cxx(:,:,i) = cxx(:,:,i) + roll(1)*eye(n);
    cuu(:,:,i) = cuu(:,:,i) + roll(2)*eye(m);
    cxu(:,:,i) = cxu(:,:,i);
end
cx(:,N) = cx(:,N) + roll(1)*x(:,N) - roll(1)*x_bar(:,N);
cxx(:,:,N) = cxx(:,:,N) + roll(1)*eye(n);


function [xnew,unew,cnew] = forward_pass(x0,u,L,x,du,Alpha,DYNCST,DYNCST2,lims,diff,roll,x_bar,u_bar,COMTask,SwingFootTask,spmode,xf)
...
for i = 1:N
    ...
    [xnew(:,:,i+1), ~]  = DYNCST(xnew(:,:,i), unew(:,:,i), COMTask(:,i*K1), SwingFootTask(:,i*K1), spmode, xf, i*K1);
    cnew(:,:,i) = DYNCST2(xnew(:,:,i), unew(:,:,i), roll, x_bar(:,i*K1), u_bar(:,i*K1),COMTask(:,i*K1),SwingFootTask(:,i*K1),spmode,xf,i*K1);
end
cnew(:,:,N+1) = DYNCST2(xnew(:,:,N+1), nan(m,K,1), roll, x_bar(:,(N+1)*K1), nan(m,K,1), COMTask(:,(N+1)*K1),SwingFootTask(:,(N+1)*K1),spmode,xf,i);
