function [x, force, u, cnew, cost] = ADMM_CEN(obj,DYNCST,DYNCST2,x0,xc0,u0,u0_cen,COMTask,SwingFootTask,spmode,xf,Op)
    ...
    trace(1).iter = 1;
    iter = 1;
    % --- initial trajectory for centroidal and whole-body 
    [x,force,un,~]  = initialtraj(x0,u0,f_init,zeros(1,6),c_init,h_init,f_init,x_init,u_init,f_init,COMTask,SwingFootTask,spmode,xf,DYNCST);
    [xc,uf] = centroidal_init(xc0,u0_cen,zeros(1,6),c_init,h_init,f_init2,COMTask,SwingFootTask,DYNCST2);
    % c01 = sum(c0(:));
    ...
    roll = [1e5 0 1e-4 10 0.1 1e-2]; 
    % roll = [1e4 0 1e-3 50 1 1e-3]; 
    alpha = 1.65;
    alphak = 1;
    yita = 0.999;
    it2 = 50;
    ...
    if spmode==1
        ufnew = [uf([2 1],:);zeros(2,size(uf,2))]; % [normal;tangential] note: different from uf
    elseif spmode==2
        ufnew = [zeros(2,size(uf,2));uf([2 1],:)]; % [normal;tangential] note: different from uf
    end
    ...
    c_lambda = com-cnew;
    h_lambda = momentum - hnew;
    uf_lambda = forcenew - ufnew;

    x_lambda = xnew - x_bar;
    u_lambda = unew - u_bar;
    force_lambda = forcenew - force_bar;

    c_lambda2 = zeros(2,size(x,2));
    h_lambda2 = zeros(3,size(x,2));
    uf_lambda2 = zeros(size(force));
    ...
    % Initialize the residual for primal and dual variables
    res_c = zeros(1,it2);
    res_h = zeros(1,it2);
    res_uf = zeros(1,it2);
    res_clambda = zeros(1,it2);
    res_mlambda = zeros(1,it2);
    res_uflambda = zeros(1,it2);

    res_u = zeros(1,it2);
    res_x = zeros(1,it2);
    res_force = zeros(1,it2);
    res_ulambda = zeros(1,it2); 
    res_xlambda = zeros(1,it2);
    res_forcelambda = zeros(1,it2);
    ...
    for i = 1:it2
        ...
        %% Original simple ADMM 
            ...
            % store the values for dynamics consensus from previous iteration
            if i==1
                Op.maxIter=100;
            elseif i>1
                Op.maxIter=15;
            ...
            if spmode==1
                % DDP iteration
                [xcnew, ufnew_s] = iLQG_CEN(DYNCST2, xc0, ufnew_s, roll, com+c_lambda, ...
                momentum+h_lambda, forcenew(1:2,:)+force_lambda(1:2,:), COMTask, SwingFootTask, Op);

                cnew = xcnew(1:2,:);
                hnew = [xcnew(5,:);M*xcnew(3:4,:)];
                hnew(1,:) = momentum(1,:)+h_lambda(1,:);
                ufnew = [ufnew_s([2 1],:);zeros(2,size(ufnew_s,2))];
            elseif spmode==2
                [xcnew, ufnew_s] = iLQG_CEN(DYNCST2, xc0, ufnew_s, roll, com+c_lambda, ...
                momentum+h_lambda, forcenew(3:4,:)+force_lambda(3:4,:), COMTask, SwingFootTask, Op);
            ...
            % Relaxation between first two blocks
            %         cnew2 = alpha*cnew + (1-alpha)*com;
            %         ufnew2 = alpha*ufnew + (1-alpha)*forcenew;
            ...
            forcenew2 = alpha*forcenew + (1-alpha)*force_bar;
            unew2 = alpha*unew + (1-alpha)*u_bar;

            %====== projection operator to satisfy the constraint
            % store the values for extra constraints from previous iteration
            x_bar_old = x_bar;
            u_bar_old = u_bar;
            force_bar_old = force_bar;
            [x_bar, force_bar, u_bar] = proj(xnew2+x_lambda, unew2+u_lambda, forcenew2+force_lambda, 0.5, Op.lims);
            ...
            x_lambda = x_lambda + xnew2 - x_bar; %rho4
            u_lambda = u_lambda + unew2 - u_bar; %rho5
            force_lambda = force_lambda + forcenew2 - force_bar; %rho6
    ...
    %         % Relaxation between first two blocks
    %         cnew2 = alpha*cnew + (1-alpha)*com;
    %         hnew2 = alpha*hnew + (1-alpha)*hnew;
    %         ufnew2 = alpha*ufnew + (1-alpha)*forcenew;
    %         
    %         %====== proximal operator to minimize to cost
    %         [xnew, forcenew, unew, ~] = iLQG_TRK(DYNCST, x0, unew, forcenew, roll, ...
    %             cnew2-c_lambda,hnew2-h_lambda,ufnew2-uf_lambda, ...
    %             x_bar-x_lambda,u_bar-u_lambda, force_bar-force_lambda, ...
    %             COMTask,SwingFootTask,spmode,xf,Op);
    %         [com, momentum] = wholebodymapping(obj,xnew);
    %         
    ...
    %         u_bar_old = u_bar;
    %         force_bar_old = force_bar;
    %         [x_bar, force_bar, u_bar] = proj(xnew+x_lambda, unew2+u_lambda, forcenew+force_lambda, 1, Op.lims);
    % 
    %         %====== dual variables update
    %         c_lambda = c_lambda + com - cnew2; %rho1
    %         h_lambda = h_lambda + momentum - hnew2; %rho2
    %         uf_lambda = uf_lambda + forcenew - ufnew2; %rho3
    %         
    %         x_lambda = x_lambda + xnew - x_bar; %rho4
    %         u_lambda = u_lambda + unew2 - u_bar; %rho5
    %         force_lambda = force_lambda + forcenew - force_bar; %rho6

            %% Accelerated ADMM based on Nestorov (strong convex)
            %====== centroidal momentum model 
            % store the values for dynamics consensus from previous iteration
    %         if i==1
    %             Op.maxIter=100;
    ...
    %         [xnew, forcenew, unew, ~] = iLQG_TRK(DYNCST, x0, unew, forcenew, roll, ...
    %             cnew-c_lambda,hnew-h_lambda,ufnew-uf_lambda, ...
    %             x_bar2-x_lambda2,u_bar2-u_lambda2, force_bar2-force_lambda2, ...
    %             COMTask,SwingFootTask,spmode,xf,Op);
    %         [com, momentum] = wholebodymapping(obj,xnew);
    %         
    %         %====== Relaxtion
    ...
    %         u_bar_old = u_bar;
    %         force_bar_old = force_bar;
    %         [x_bar, force_bar, u_bar] = proj(xnew+x_lambda2, unew+u_lambda2, forcenew+force_lambda2, 1, Op.lims);
    % 
    %         %====== dual variables update
    %         
    %         c_lambda = c_lambda + com - cnew; %rho1
    ...
    %         force_bar2 = force_bar + 0.375*((alphak_v(:,i)-1)/alphak_v(:,i+1))*(force_bar-force_bar_old);
    %         
    ...
    %         force_lambda2 = force_lambda + 0.375*((alphak_v(:,i)-1)/alphak_v(:,i+1))*(force_lambda-force_lambda_old);
    %%==================================================
        else    
            %% Accelerated ADMM based on Nestorov (strong convex)
    %         %====== centroidal momentum model 
    ...
    %         unew2 = alpha*unew + (1-alpha)*u_bar;
            ...
            %====== Nestorov step
            alphak_v(:,i+1) = (1+sqrt(1+4*alphak_v(:,i)^2))/2;
            x_bar2 = x_bar + (alphak_v(:,i)-1)/alphak_v(:,i+1)*(x_bar-x_bar_old);
            u_bar2 = u_bar + (alphak_v(:,i)-1)/alphak_v(:,i+1)*(u_bar-u_bar_old);
            x_lambda2 = x_lambda + (alphak_v(:,i)-1)/alphak_v(:,i+1)*(x_lambda-x_lambda_old);
            u_lambda2 = u_lambda + (alphak_v(:,i)-1)/alphak_v(:,i+1)*(u_lambda-u_lambda_old);
    ...
    %         if res_uf(:,i)^2 > 10*res_uflambda(:,i)^2
    %             roll(3) = 2*roll(3);
    ...
    %         if res_x(:,i)^2 > 8*res_xlambda(:,i)^2
    %             roll(4) = 5*roll(4);
    %             x_lambda = x_lambda/5;
    %         elseif res_xlambda(:,i)^2 > 8*res_x(:,i)^2
    %             roll(4) = roll(4)/5;
    %             x_lambda = x_lambda*5;
    %         end

            if res_u(:,i)^2 > 10*res_ulambda(:,i)^2
                roll(5) = 4*roll(5);
                u_lambda = u_lambda/4;
            elseif res_ulambda(:,i)^2 > 10*res_u(:,i)^2
                roll(5) = roll(5)/4;
                u_lambda = u_lambda*4;
            end

            if res_force(:,i)^2 > 10*res_forcelambda(:,i)^2
                roll(6) = 8*roll(6);
                force_lambda = force_lambda/8;
            elseif res_forcelambda(:,i)^2 > 10*res_force(:,i)^2
                roll(6) = roll(6)/8;
                force_lambda = force_lambda*8;
            end
        end
    ...
    plot(l,res_clambda,'DisplayName','residue dual com');
    plot(l,res_mlambda,'DisplayName','residue dual momentum');
    plot(l,res_uflambda,'DisplayName','residue dual consensus force');
    ...
    elseif spmode==2
        plot(ii,forcenew(3,:),'DisplayName','new norm');
    end
    hold on
    if spmode==1
        plot(ii,forcenew(2,:),'DisplayName','new tang');
        plot(ii,ufnew(1,:),'DisplayName','centroidal norm');
        plot(ii,ufnew(2,:),'DisplayName','cenroidal tang');
    elseif spmode==2
        plot(ii,forcenew(4,:),'DisplayName','new norm');
        plot(ii,ufnew(3,:),'DisplayName','centroidal norm');
        plot(ii,ufnew(4,:),'DisplayName','cenroidal tang');
    end    
    ...
    subplot(2,2,4)
    % aa = 1:N+1;
    plot(com(1,:),com(2,:),'DisplayName','cenroidal CoM');
    hold on
    plot(cnew(1,:),cnew(2,:),'DisplayName','whole-body CoM');
...
function [com, momentum] = wholebodymapping(obj,xnew)
    ...
    for i = 1:N
        ...
        momentum0 = centroidalMomentumMatrix(obj,xnew(1:6,i))*xnew(7:12,i);
        momentum(:,i) = momentum0(2:2:6,:);
    end


function [xcnew, ufnew] = centroidal_init(xc0,uf,roll,c_bar,h_bar,uf_bar,COMTask,SwingFootTask,DYNCST)
    n        = size(xc0,1);
    m        = size(uf,1);
    N        = size(uf,2);
    ...
    ufnew        = uf;
    for i = 1:N
        [xcnew(:,i+1),~] = DYNCST(xcnew(:,i), ufnew(:,i),roll,...
            c_bar(:,i),h_bar(:,i),uf_bar(:,i),COMTask(:,i),SwingFootTask(:,i),i);
    end
...
defaults = {'lims',           [],...            control limits
            ...
            'tolGrad',        1e-6,...          gradient exit criterion11111111
...
function [x, u, L, Vx, Vxx, cost, trace, stop] = iLQG_CEN(DYNCST, ...
    x0, u0, roll, c_bar, h_bar, uf_bar, ...
    COMTask,SwingFootTask,Op)
%---------------------- user-adjustable parameters ------------------------
defaults = {'lims',           [],...            control limits
            'parallel',       true,...          use parallel line-search?
            'Alpha',          10.^linspace(0,-3,11),... backtracking coefficients
...
if size(x0,2) == 1
    diverge = true;
    for alpha = Op.Alpha
        [x,un,cost]  = forward_pass2(x0(:,1),alpha*u,[],[],[],1,...
            DYNCST,Op.lims,[],roll,c_bar, h_bar, uf_bar, ...
            COMTask,SwingFootTask);
        % simplistic divergence test
        if all(abs(x(:)) < 1e8)
            u = un;
            diverge = false;
...
for iter = 1:Op.maxIter
    ...
    if flgChange
        ...
        [~,~,fx,fu,fxx,fxu,fuu,cx,cu,cxx,cxu,cuu]   = DYNCST(x,[u nan(m,1)],...
            roll, c_bar, h_bar, [uf_bar nan(m,1)],...
            COMTask,SwingFootTask,1:N+1);
%         [cx,cu,cf,cff,cxx,cxu,cuu] = preprocessing(x,u,roll,cx,cu,cxx,cxu,cuu,uf_bar,x_bar,u_bar,f_bar);
        trace(iter).time_derivs = toc(t_diff);
        flgChange   = 0;
    end
    ...
    while ~backPassDone
        ...
        t_back   = tic;
        [diverge, Vx, Vxx, l, L, dV] = back_pass2(cx,cu,cxx,cxu,cuu,fx,fu,fxx,fxu,fuu,lambda,Op.regType,Op.lims,u);
        trace(iter).time_backward = toc(t_back);
    ...
    if backPassDone
        t_fwd = tic;
        if Op.parallel  % parallel line-search
            [xnew,unew,costnew] = forward_pass2(x0 ,u, L, x(:,1:N), l,...
                Op.Alpha, DYNCST,Op.lims,Op.diffFn,roll, ...
                c_bar, h_bar, uf_bar, ...
                COMTask,SwingFootTask);
            Dcost               = sum(cost(:)) - sum(costnew,2);
            [dcost, w]          = max(Dcost);
            ...
            for alpha = Op.Alpha
                [xnew,unew,costnew]   = forward_pass(x0 ,u+l*alpha, L, x(:,1:N),[],1,...
                    DYNCST,lims,Op.diffFn,roll,...
                    c_bar, h_bar, uf_bar, COMTask,SwingFootTask);
                dcost    = sum(cost(:)) - sum(costnew(:));
                expected = -alpha*(dV(1) + alpha*dV(2));
                if expected > 0
                    z = dcost/expected;
...
function [xnew,unew,cnew] = forward_pass2(x0,u,L,x,du,Alpha,DYNCST,lims,diff,...
    roll,c_bar,h_bar,uf_bar,COMTask,SwingFootTask)
% parallel forward-pass (rollout)
% internally time is on the 3rd dimension, 
% to facillitate vectorized dynamics calls
...
for i = 1:N
    ...
    [xnew(:,:,i+1), cnew(:,:,i)]  = DYNCST(xnew(:,:,i), unew(:,:,i), roll, ...
        c_bar(:,i*K1),h_bar(:,i*K1),uf_bar(:,i*K1),...
        COMTask(:,i*K1), SwingFootTask(:,i*K1), i*K1);
%     cnew(:,:,i) = DYNCST2(xnew(:,:,i), unew(:,:,i), roll, x_bar(:,i*K1), u_bar(:,i*K1),COMTask(:,i*K1),SwingFootTask(:,i*K1),spmode,xf,i*K1);
end
[~,cnew(:,:,N+1)] = DYNCST(xnew(:,:,N+1), nan(m,K,1), roll, ...
    c_bar(:,(N+1)*K1),h_bar(:,(N+1)*K1),nan(m,K,1), ...
    COMTask(:,(N+1)*K1),SwingFootTask(:,(N+1)*K1), i);
% put the time dimension in the columns
xnew = permute(xnew, [1 3 2]);
unew = permute(unew, [1 3 2]);
cnew = permute(cnew, [1 3 2]);
...
function [diverge, Vx, Vxx, k, K, dV] = back_pass2(cx,cu,cxx,cxu,cuu,fx,fu,fxx,fxu,fuu,lambda,regType,lims,u)
% Perform the Ricatti-Mayne backward pass
