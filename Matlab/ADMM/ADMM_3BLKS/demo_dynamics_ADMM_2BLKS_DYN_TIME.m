function [x,u] = demo_dynamics_ADMM_2BLKS_DYN_TIME(Op)
...
dt_dyn   = 0.02 * ones(1, size(x_des,2)); 
xd       = [q_des; zeros(7, size(x_des,2)); zeros(2, size(x_des,2)) ;x_des(end,:); dt_dyn];
...
[x,u]= ADMM_DDP_2BLKS_DYN_TIME(DYNCST, x0, u0, Op);
...
function y = robot_dynamics(x, u, i)
    ...
    % states - velocity
    xd  = x(8:14, :, :);
    xdd = fdyn_dynamics_admm_2blk_dyn_time(x, u, RC_d, K_d);
...
function [c] = admm_robot_cost(x, u, i, rhao, x_bar, c_bar, u_bar, thetalist_bar, thetalistd_bar)
    ...
    % cu  = [5e-10 * ones(1, 7) 0.4];         % control cost coefficients
    ...
    % cx  = 5e-1 * [0.0*ones(1,7) 0.0001*ones(1,7) 0.000000 0.0001 5 0.001];           % running cost coefficients
    ...
    x_d_c = repmat(x_des(:,i), 1, size(x,2) / numel(i));
    ...
    % err_fk  = pa - x(8:10,:);
    err_fk  = sum((pa - x_d_c(1:3,:)).^2,1);
    
    % final cost
    if any(final)
       llf      = cx_w(15:17) * (x(15:17,final)-x_d(15:17,final)).^2; 
        %        llf      = 0;
       lf       = double(final);
       lf(final)= llf;
    else
    ...
    % running cost    
    lx     = cx_w(15:17) * (x(15:17,:)-x_d(15:17,:)).^2 + (rhao(1)/2)*ones(1,18)*(x(:,:)-x_bar(:,:)).^2 + ...
        (rhao(3)/2)*ones(1,2)*(cen-c_bar).^2; 
